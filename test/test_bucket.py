...
def list_local_files(path):
    ...
    for root, _, files in os.walk(str(path)):
        for file_name in files:
            ...
            relative_file_path = os.path.relpath(full_path, str(path)).replace(os.sep, '/')
            set_of_filenames.add((relative_file_path, compute_etag(full_path)))
...
class BucketWithCopyCounter(Bucket):
    def __init__(self, connection, name, create=True):
        super(BucketWithCopyCounter, self).__init__(connection, name, create)
        self._nbr_of_copies = 0

    def copy_file(self, source, dest):
        self._nbr_of_copies += 1
        super(BucketWithCopyCounter, self).copy_file(source, dest)


# ================================== Tests using Moto ==================================
@pytest.mark.skipif(not _has_moto, reason="moto not available for this Python version")
class TestBucketPublicMethodsMoto:

    @mock_aws
    def test_sync_files_avoid_unnecessary_copies(self, tmpdir):
        # cf QNET-5274
        bucket_name = "dolly"
        ...
        write_in(os.path.join(tmp_path, "local1"), 'Tu ne copieras point sur ton voisin')
        write_in(os.path.join(tmp_path, "local2"), 'Tu ne copieras point sur ton voisin')

        # Synchronize the content of this folder with the bucket
        bucket.sync_directory(tmp_path)

        # Check that it's indeed synchronized
        local_files = list_local_files(tmp_path)  # tmp_path is already a string
        bucket_files = set()
