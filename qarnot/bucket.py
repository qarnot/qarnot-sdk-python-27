...
class Bucket(Storage):  # pylint: disable=W0223
    ...
    def __init__(self, connection, name, create=True, filtering=None, resources_transformation=None, cacheTTLSec=None):
        ...
        self._uuid = name
        self._filtering = filtering or Filtering()
        self._resources_transformation = resources_transformation or ResourcesTransformation()
        self._cache_ttl_sec = cacheTTLSec
    ...
    def delete(self):
        ...
        try:
            ...
            raise MissingBucketException("Cannot delete {}. Bucket not found.".format(err.response.get('Error').get('BucketName')))
    ...
    def list_files(self):
        ...
        try:
            ...
            raise MissingBucketException("Cannot list files. Bucket {} not found.".format(err.response.get('Error').get('BucketName')))
    ...
    def sync_remote_to_local(self, local_directoy, remote_directory=None):
        ...
        try:
            ...
            raise MissingBucketException("Cannot synchronize. Bucket {} not found.".format(err.response.get('Error').get('BucketName')))
    ...
    def sync_files(self, files, verbose=False, remote=None):
        ...
        try:
            ...
            raise MissingBucketException("Cannot sync files. Bucket {} not found.".format(err.response.get('Error').get('BucketName')))
    ...
    def add_string(self, string, remote):
        ...
        if isinstance(string, bytes):
            self.add_file(io.BytesIO(string), remote)
        else:
            self.add_file(io.BytesIO(string.encode('utf-8')), remote)
    ...
    def add_file(self, local_or_file, remote=None):
        ...
        try:
            ...
            raise MissingBucketException("Cannot add string. Bucket {} not found.".format(err.response.get('Error').get('BucketName')))
        finally:
            if tobeclosed:
                file_.close()
    ...
    def copy_file(self, source, dest):
        try:
            ...
            raise MissingBucketException("Cannot copy file {} to {} from bucket {}. Bucket not found.".format(source, dest, err.response.get('Error').get('BucketName')))
        ...
        with open(local, 'wb') as data:
            if hasattr(remote, 'read'):
                ...
                try:
                    ...
                    raise MissingBucketException("Cannot download file {} from bucket {}. Bucket not found.".format(remote, err.response.get('Error').get('BucketName')))
        return local
    ...
    def delete_file(self, remote):
        try:
            ...
            raise MissingBucketException("Cannot delete file {} from bucket {}. Bucket not found.".format(remote, err.response.get('Error').get('BucketName')))
